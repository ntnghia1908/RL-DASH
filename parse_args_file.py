...
def parse():
    parser = argparse.ArgumentParser(
        description="DASH singlepath evaluation. Hyperparameters are set to the best.",
    ...
    parser.add_argument(
        ...
        type=int,
        default=15000,
        help="Episodes to evaluate the RL algorithms",
    )
    parser.add_argument(
        "--log_qoe", type=str2bool, default=True, help="whether to use log QoE"
    )
    parser.add_argument(
        "--dataset",
        type=str,
    ...
    parser.add_argument(
        "--seed", type=int, default=int(time.time()), help="global random seed"
    )
    parser.add_argument(
        "--algorithms",
        type=str,
        choices=["DQN", "A2C", "PPO", "None"],
        default="None",
    ...
    parser.add_argument(
        "--use_wandb", type=str2bool, default=False, help="whether to use wandb online"
    )
    ...
    dqn.add_argument(
        "--dqn_lr", type=float, default=0.000519797694783514, help="DQN learning rate"
    )
    dqn.add_argument(
        "--dqn_buffer_size", type=int, default=7880, help="DQN max buffer size"
    )
    dqn.add_argument(
        "--dqn_learning_starts",
        type=int,
        default=2323,
        help="DQN min buffer size to start learning",
    )
    dqn.add_argument("--dqn_batch_size", type=int, default=295, help="DQN batch size")
    dqn.add_argument(
        "--dqn_tau", type=float, default=1.0, help="DQN Polyak update coefficient"
    )
    dqn.add_argument(
        "--dqn_gamma", type=float, default=0.99, help="DQN discount factor"
    )
    dqn.add_argument(
        "--dqn_train_freq",
        type=int,
        default=42,
        help="DQN update the model every dqn_train_freq steps",
    )
    dqn.add_argument(
        "--dqn_grad_steps",
        type=int,
        default=2,
        help="DQN gradient step each rollout. "
    ...
    a2c.add_argument(
        "--a2c_lr", type=float, default=3.308224595892156e-05, help="A2C learning rate"
    )
    a2c.add_argument(
        "--a2c_n_steps",
        type=int,
        default=192,
        help="A2C num steps to run for each env each update",
    )
    a2c.add_argument(
        "--a2c_gamma", type=float, default=0.99, help="A2C discount factor"
    )
    ...
    a2c.add_argument(
        "--a2c_vf_coef",
        type=float,
        default=0.32790564221824087,
        help="A2C value function coef",
    )
    a2c.add_argument(
        "--a2c_max_grad_norm",
        type=float,
        default=0.5,
        help="A2C max value for grad clip",
    )
    a2c.add_argument(
        "--a2c_rmsprop",
        type=str2bool,
        default=True,
        help="whether to use RMSProp or Adam as optimizer",
    )
    a2c.add_argument(
        "--a2c_norm_adv",
        type=str2bool,
        default=True,
        help="whether to use normalized advantage",
    )

    # PPO parameters
    ppo = parser.add_argument_group("PPO parameters")
    ppo.add_argument(
        "--ppo_lr", type=float, default=1.0676995458551102e-05, help="PPO learning rate"
    )
    ppo.add_argument(
        "--ppo_n_steps_coef",
        type=int,
    ...
    ppo.add_argument(
        "--ppo_n_epochs", type=int, default=10, help="PPO num epochs to run each update"
    )
    ppo.add_argument(
        "--ppo_gamma", type=float, default=1, help="PPO reward discount factor"
    )
    ppo.add_argument(
        "--ppo_gae_lambda", type=float, default=0.95, help="PPO GAE lambda coef"
    )
    ...
    ppo.add_argument(
        ...
        type=float,
        default=None,
        help="PPO Value function clipping parameter",
    )
    ppo.add_argument(
        "--ppo_ent_coef", type=float, default=0.00001, help="PPO entropy coef"
    )
    ppo.add_argument(
        "--ppo_vf_coef",
        type=float,
        default=0.30567628766494326,
        help="PPO value function coef",
    )
    ppo.add_argument(
        "--ppo_max_grad_norm",
        type=float,
        default=0.5,
        help="PPO Gradient clipping value",
    )
