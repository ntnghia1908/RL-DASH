...
import numpy as np
import load_trace_v1 as load_trace
...
BITS_IN_BYTE = 8.0
RANDOM_SEED = 42
DELAY_FACTOR = 3  # unused
VIDEO_CHUNCK_LEN = 4000.0  # (ms), every time add this amount to buffer
BITRATE_LEVELS = 7
TOTAL_VIDEO_CHUNCK = 60
PACKET_PAYLOAD_PORTION = 1  # Original: 0.95
LINK_RTT = 80  # (sec)
PACKET_SIZE = 1500  # bytes (unused)
DT = 1  # time granularity
VIDEO_BIT_RATE = [700, 900, 2000, 3000, 5000, 6000, 8000]  # Kbps
VIDEO_BIT_RATE = np.log(np.array(VIDEO_BIT_RATE) / 700)
DEFAULT_QUALITY = 0
M_IN_K = 1000.0
...
def main():
    ...
    import pickle
    video_br = VideoListCollector(save_dir="../video_list_bunny8k").get_trace_matrix([700, 900, 2000, 3000, 5000, 6000, 8000])
    video_size = {x: y for x in range(len(video_br)) for y in video_br[:, :TOTAL_VIDEO_CHUNCK]}
    ...
    cooked_time = np.array(list(range(fcc_test.shape[1])))
    cooked_bw = fcc_test[2] / 1e6
    # print({x: y for x in cooked_time for y in cooked_bw})
                ...
                for m in range(BITRATE_LEVELS):
                    if (n - 1, t, b, m) in total_reward:
                        for new_bit_rate in range(BITRATE_LEVELS):
                            ...
                            # TODO: REWARD FUNCTION, REBUF PENALTY might require its norm factor
                            r = VIDEO_BIT_RATE[new_bit_rate] \
                                - REBUF_PENALTY * rebuf \
    ...
    print(optimal_total_reward / 100)
